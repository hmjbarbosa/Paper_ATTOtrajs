...
nxy[nxy<5]=np.nan
wxy = np.histogram2d(y[mask], x[mask], bins=[yedges, xedges],weights=swh[mask])[0]
meanval = wxy/nxy
...
norm = matplotlib.colors.BoundaryNorm(np.arange(0,5000,500), cmap.N)

pc = ax1.pcolormesh(xedges, yedges, meanval, cmap=cmap, norm=norm) #, vmin=-6., vmax=24.
fig.colorbar(pc, orientation='horizontal',pad=0.1,aspect=30,
             label='SW Rad. Acc. [W.h/m2]',fraction=0.06,shrink=0.9)

#CS = ax1.contour(xcenters, ycenters, meanval,colors='red', levels=np.arange(0,5000,500))
...
# export PNG
fname = 'figs_time/histplot2d_traj_all_100_500_24h_swh.png'
plt.savefig(fname)
#
...
plt.figure(2)
plt.clf()
plt.hist(swh[mask])#,bins=np.arange(-15.5,29.5,1))
