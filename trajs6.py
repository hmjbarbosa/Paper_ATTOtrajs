...
nxy[nxy<5]=np.nan
wxy = np.histogram2d(y[mask], x[mask], bins=[yedges, xedges],weights=sw[mask])[0]
meanval = wxy/nxy
...
norm = matplotlib.colors.BoundaryNorm(np.arange(0,1000,50), cmap.N)
...
fig.colorbar(pc, orientation='horizontal',pad=0.1,aspect=30,
             label='SW Rad. Inst. [W/m2]',fraction=0.06,shrink=0.9)

#CS = ax1.contour(xcenters, ycenters, meanval,colors='red', levels=np.arange(0,1000,50))
...
# export PNG
fname = 'figs_time/histplot2d_traj_all_100_500_24h_swinst.png'
plt.savefig(fname)
#
...
plt.figure(2)
plt.clf()
plt.hist(sw[mask])#,bins=np.arange(-15.5,29.5,1))
