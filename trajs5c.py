...
nxy[nxy<1]=np.nan
wxy = np.histogram2d(y[mask], x[mask], bins=[yedges, xedges],weights=t0[mask])[0]
meant0 = wxy/nxy
...
norm = matplotlib.colors.BoundaryNorm(np.arange(-6,26,2), cmap.N)

pc = ax1.pcolormesh(xedges, yedges, meant0, vmin=-6., vmax=24., cmap=cmap, norm=norm)
fig.colorbar(pc, orientation='horizontal',pad=0.1,aspect=30,
             label='Trajectory start time [h, UTC]',fraction=0.06,shrink=0.9)

CS = ax1.contour(xcenters, ycenters, meant0,colors='red', levels=np.arange(0,18,2))
...
# export PNG
fname = 'figs_time/histplot2d_traj_all_100_500_24h_t0.png'
plt.savefig(fname)
#
...
plt.figure(2)
plt.clf()
plt.hist(t0[mask],bins=np.arange(-15.5,29.5,1))
