...
# selecao dos dados
mask = (alt>=100) & (alt<=500)
nxy = np.histogram2d(y[mask], x[mask], bins=[yedges, xedges])[0]
nxy[nxy<5]=np.nan
wxy = np.histogram2d(y[mask], x[mask], bins=[yedges, xedges],weights=dt[mask])[0]
meandt = wxy/nxy
...
#norm = matplotlib.colors.BoundaryNorm(np.arange(0,18,2), cmap.N)

pc = ax1.pcolormesh(xedges, yedges, meandt, vmin=0., vmax=18.)#, cmap=cmap, norm=norm)
fig.colorbar(pc, orientation='horizontal',pad=0.1,aspect=30,
             label='Trajectory travel time [h]',fraction=0.06,shrink=0.9)

CS = ax1.contour(xcenters, ycenters, meandt,colors='red', levels=np.arange(2,18,2))
ax1.clabel(CS, CS.levels, inline=True, fmt='%1.0fh', fontsize=10)

ax1.set_title('100-500m, 0-23h UTC')
...
# export PNG
fname = 'figs_time/histplot2d_traj_all_100_500_24h_dt.png'
plt.savefig(fname)
#
...
#plt.plot(np.mean(x[mask]), np.mean(y[mask]), 'or')
plt.figure(2)
plt.clf()
plt.hist(dt[mask],bins=np.arange(-15.5,29.5,1))
